...
def traiter_email(nom_prenom, email, adresse, liens, sujets, noms, smtp_configs, html_template, total, speed, progress, task_id):
    ...
    with lock:
        ...
        mail_from = smtp_config['user']  # Use the user field as the sender email

    link = random.choice(liens)
    html = html_template.replace("{LINK}", link).replace("{ADDRESS}", adresse).replace("{NOM_PRENOM}", nom_prenom)

    ok = envoyer_email(smtp_config, mail_from, sender_name, subject, email, html, link)
    ...
    with lock:
        ...
        with open("log.txt", "a", encoding="utf-8") as log:
            log.write(f"{datetime.now()} | {email} | {'OK' if ok else 'FAIL'}\n")

        console.print(
            f"[bold green]✔ {email}[/bold green] — "
            f"[blue]{link}[/blue] — "
            f"[magenta]{mail_from}[/magenta] — "
